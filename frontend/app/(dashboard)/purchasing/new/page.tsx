...
export default function NewPurchaseOrderPage() {
  ...
  const submit = async () => {
    ...
    const validItems = items.filter(it => it.product_id && it.quantity && it.unit_price_orig);
    if (!validItems.length) { setError('請至少填寫一項採購品項'); return; }
    setSaving(true);
    setError('');
    try {
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">新增採購單</h1>
      ...
      <div className="card">
        ...
        <table>
          <thead><tr><th>商品</th><th>數量</th><th>單價 (原幣)</th><th>小計</th><th></th></tr></thead>
          <tbody>
            {items.map((it, i) => (
              <tr key={i}>
                <td>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginTop: '1rem' }}>
        <a href="/purchasing" className="btn btn-outline">取消</a>
        <button className="btn btn-primary" onClick={submit} disabled={saving}>{saving ? '建立中...' : '建立採購單'}</button>
      </div>
    </div>
  );
}
